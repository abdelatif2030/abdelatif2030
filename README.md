<div align="center">

# Abdelatef Mohamed
### Cloud & DevOps Engineer

*Designing, deploying, and automating scalable, secure, production-ready infrastructure*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abdo-mohamed-94b3433a4/)
[![Upwork](https://img.shields.io/badge/Upwork-6FDA44?style=for-the-badge&logo=upwork&logoColor=white)](https://www.upwork.com/freelancers/~01776e1536dd083cdd)
[![Fiverr](https://img.shields.io/badge/Fiverr-1DBF73?style=for-the-badge&logo=fiverr&logoColor=white)](https://www.fiverr.com/sellers/abdelatif2030)

</div>

---

## About Me

I'm a Cloud & DevOps Engineer with hands-on production experience across **AWS**, **Huawei Cloud Stack (HCS)**, and enterprise environments. My work focuses on eliminating manual operations, accelerating deployments, and building infrastructure that scales reliably under real-world conditions.

- Currently working with **Huawei Cloud Stack (HCS)** in multi-tenant enterprise environments
- Experienced in **AWS cloud-native architecture** — EC2, ECS, EKS, ECR, Glue, S3, Athena
- Building production-grade systems with **Infrastructure as Code, CI/CD pipelines, and full observability stacks**
- Security-first mindset: IAM least-privilege, SSH hardening, firewall config, audit logging

---

## Tech Stack

### Cloud Platforms
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![Huawei Cloud](https://img.shields.io/badge/Huawei_Cloud-FF0000?style=flat-square&logo=huawei&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)

### Containers & Orchestration
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Amazon EKS](https://img.shields.io/badge/EKS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)
![Amazon ECS](https://img.shields.io/badge/ECS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)

### CI/CD & Automation
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat-square&logo=jenkins&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![GitLab CI](https://img.shields.io/badge/GitLab_CI-FC6D26?style=flat-square&logo=gitlab&logoColor=white)
![ArgoCD](https://img.shields.io/badge/ArgoCD-EF7B4D?style=flat-square&logo=argo&logoColor=white)

### Infrastructure as Code
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat-square&logo=ansible&logoColor=white)
![CloudFormation](https://img.shields.io/badge/CloudFormation-FF4F00?style=flat-square&logo=amazonaws&logoColor=white)

### Monitoring & Observability
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat-square&logo=grafana&logoColor=white)
![Elastic Stack](https://img.shields.io/badge/ELK_Stack-005571?style=flat-square&logo=elasticstack&logoColor=white)

### Systems & Scripting
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

---

## Core Competencies

```
Cloud Architecture        ████████████████████  AWS · HCS · Azure
CI/CD Pipelines           ███████████████████░  Jenkins · GitHub Actions · ArgoCD
Container Orchestration   ██████████████████░░  Kubernetes · EKS · ECS · Docker
Infrastructure as Code    ██████████████████░░  Terraform · Ansible · CloudFormation
Monitoring & Observability █████████████████░░░  Prometheus · Grafana · ELK
Linux & Automation        ████████████████████  Bash · PowerShell · Python
Security & Networking     █████████████████░░░  IAM · VPC · Firewalls · SSH
```

---

## What I Deliver

| Area | Details |
|---|---|
| **Cloud Infrastructure** | Scalable, cost-optimized architectures on AWS and enterprise clouds |
| **CI/CD Automation** | End-to-end pipelines from commit to production with zero manual steps |
| **Container Platforms** | Kubernetes clusters, Helm deployments, EKS/ECS workloads |
| **IaC Provisioning** | Repeatable, version-controlled infrastructure with Terraform & Ansible |
| **Observability** | Full-stack monitoring: metrics, logs, dashboards, and alerting |
| **Security Hardening** | IAM least-privilege, SSH controls, Security Groups, audit logging |

---

## Let's Work Together

I'm open to **freelance projects**, **DevOps consulting**, and **full-time opportunities**.

If you need reliable cloud infrastructure, automated pipelines, or a production-ready system built right the first time — let's talk.

[![LinkedIn](https://img.shields.io/badge/Connect_on_LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abdo-mohamed-94b3433a4/)
[![Upwork](https://img.shields.io/badge/Hire_on_Upwork-6FDA44?style=for-the-badge&logo=upwork&logoColor=white)](https://www.upwork.com/freelancers/~01776e1536dd083cdd)
[![Fiverr](https://img.shields.io/badge/Order_on_Fiverr-1DBF73?style=for-the-badge&logo=fiverr&logoColor=white)](https://www.fiverr.com/sellers/abdelatif2030)

---

<div align="center">
<sub>Open to collaboration · Available for freelance · Building in public</sub>
</div>
